-- Update notify-rsvp to require service role authentication and accept rsvpId
-- The function will be updated to require service role key and fetch RSVP data server-side

-- Update insert_rsvp_with_capacity_check to call notify-rsvp via pg_net
-- This ensures notifications only happen for real RSVPs created through the proper flow
...
BEGIN
  ...
  -- SECURITY FIX: Send RSVP notification from server-side (not client)
  -- This ensures only real RSVPs trigger notifications
  BEGIN
    PERFORM net.http_post(
      url := current_setting('app.supabase_url', true) || '/functions/v1/notify-rsvp',
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Failed to send RSVP notification: %', SQLERRM;
  END;
