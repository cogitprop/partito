-- Fix the insert_rsvp_with_capacity_check function to properly bypass RLS
-- The function is SECURITY DEFINER but still needs to set the search_path 
-- and we need to ensure it can read from the rsvps table
...
BEGIN
  ...
  -- Check for existing RSVP by fingerprint AND event_id (more specific)
  SELECT * INTO v_existing_rsvp 
  FROM rsvps 
  WHERE event_id = p_event_id AND fingerprint = v_fingerprint;
