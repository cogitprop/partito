-- Fix notify-rsvp trigger to use hardcoded URL for Lovable Cloud compatibility
-- Also updates max_plus_ones default and validation

-- First, update max_plus_ones default to 3 for new events
ALTER TABLE public.events ALTER COLUMN max_plus_ones SET DEFAULT 3;

-- Update the insert_rsvp_with_capacity_check function with hardcoded URL
...
DECLARE
  ...
  -- TODO: Update this URL with your Supabase project URL
...
BEGIN
  ...
  -- Send RSVP notification using pg_net with hardcoded URL for Lovable Cloud
  BEGIN
    PERFORM net.http_post(
      url := v_supabase_url || '/functions/v1/notify-rsvp',
