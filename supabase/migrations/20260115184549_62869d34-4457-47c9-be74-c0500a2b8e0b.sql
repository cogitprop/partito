-- Fix 1: Update update_event_with_token to NOT store plaintext passwords
...
BEGIN
  ...
  -- Update the event (NO LONGER storing plaintext password)
  UPDATE events SET
    ...
    -- SECURITY FIX: Clear plaintext password when password is updated (only store hash)
    password = CASE WHEN p_updates ? 'password' THEN NULL ELSE password END,
...
-- Fix 2: Update event_has_password to check password_hash field
...
-- Fix 3: Clear existing plaintext passwords where hash exists
UPDATE events 
SET password = NULL 
