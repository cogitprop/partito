-- Fix "relation upserted does not exist" error and also avoid the INTO limitation
-- We return the upserted row as json and hydrate it into a rsvps record.
...
BEGIN
  ...
  SELECT row_to_json(upserted.*)::jsonb, inserted
    INTO v_row, v_was_insert
  FROM upserted;

  v_result := jsonb_populate_record(NULL::rsvps, v_row);
  ...
  RETURN jsonb_build_object(
    ...
    'was_updated', NOT v_was_insert,
    'was_waitlisted', v_final_status = 'waitlist' AND p_status = 'going',
    'notify_rsvp_id', CASE WHEN v_was_insert THEN v_result.id::text ELSE NULL END
  );
END;
$function$;